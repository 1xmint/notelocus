...
#: The launcher. `python -m` rather than a console script, because the script is
#: only on PATH if the package was installed with pip and the point of this is
#: that it works after `pip install -e .` in a checkout.
LAUNCHER = """@echo off
rem Generated by `notelocus shortcut`. Double-click to file loose desktop notes.
python -m notelocus.cli tidy
echo.
pause
"""
